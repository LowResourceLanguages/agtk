  ...
  throw (agfio::LoadError)
{
  ...
  if (label == "") {
    ...
    if (!sigSet.empty()) {
      ancrId = CreateAnchor(agId, 0.0, unit, sigSet);
      prev_ancrId = CreateAnchor(agId, atof(ref.c_str()), unit, sigSet);
    }
    else {
      ancrId = CreateAnchor(agId);
      prev_ancrId = CreateAnchor(agId);
      SetAnchorOffset(ancrId, 0.0);
      SetAnchorOffset(prev_ancrId, atof(ref.c_str()));
    }

    annoId = CreateAnnotation(agId, ancrId, prev_ancrId, ann_type);
    SetFeature(annoId, "label", label);
  }
